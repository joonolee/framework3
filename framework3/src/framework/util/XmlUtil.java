...
public class XmlUtil {
	...
	public static int render(HttpServletResponse response, RecordSet rs, String encoding) {
		return _setRecordSet(response, rs, encoding);
	}
	...
	public static int render(HttpServletResponse response, ResultSet rs, String encoding) {
		return _setResultSet(response, rs, encoding);
	}
	...
	public static String render(List<Map<String, Object>> mapList, String encoding) {
		return _format(mapList, encoding);
	}
	 ...
	 * RecordSet을 xml 형식으로 출력한다. (xml 헤더포함)
	 */
	private static int _setRecordSet(HttpServletResponse response, RecordSet rs, String encoding) {
		if (rs == null) {
			return 0;
		}
		PrintWriter pw;
	 ...
	 * RecordSet을 xml 형식으로 변환한다. (xml 헤더포함)
	 */
	private static String _format(RecordSet rs, String encoding) {
		if (rs == null) {
			return null;
		}
		StringBuilder buffer = new StringBuilder();
		buffer.append(_xmlHeaderStr(encoding));
		buffer.append(_format(rs));
		return buffer.toString();
	}
	 ...
	 * ResultSet을 xml 형식으로 출력한다 (xml 헤더포함).
	 */
	private static int _setResultSet(HttpServletResponse response, ResultSet rs, String encoding) {
		if (rs == null) {
			return 0;
		}
		try {
	 ...
	 * ResultSet을 xml 형식으로 변환한다 (xml 헤더포함).
	 */
	private static String _format(ResultSet rs, String encoding) {
		if (rs == null) {
			return null;
		}
		StringBuilder buffer = new StringBuilder();
	 ...
	 * Map객체를 xml 형식으로 변환한다 (xml 헤더포함).
	 */
	private static String _format(Map<String, Object> map, String encoding) {
		if (map == null) {
			return null;
		}
		StringBuilder buffer = new StringBuilder();
		buffer.append(_xmlHeaderStr(encoding));
		buffer.append(_format(map));
		return buffer.toString();
	}
	 ...
	 * List객체를 xml 형태로 변환한다 (xml 헤더포함).
	 */
	private static String _format(List<Map<String, Object>> mapList, String encoding) {
		if (mapList == null) {
			return null;
		}
		StringBuilder buffer = new StringBuilder();
		buffer.append(_xmlHeaderStr(encoding));
		buffer.append(_format(mapList));
		return buffer.toString();
	}
		...
		for (Entry<String, Object> entry : map.entrySet()) {
			...
			if (value == null) {
				...
				if (value instanceof Number) {
					...
					buffer.append("<" + key.toLowerCase() + ">" + _format((Map<String, Object>) value) + "</" + key.toLowerCase() + ">");
				} else if (value instanceof List) {
					buffer.append("<" + key.toLowerCase() + ">" + _format((List<Map<String, Object>>) value) + "</" + key.toLowerCase() + ">");
				} else {
